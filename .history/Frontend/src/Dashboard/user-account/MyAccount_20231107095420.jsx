...
const MyAccount = () => {
  ...
  return (
    <div className='max-w-[1170px] px-5 mt-[100px] mb-[50px] mx-auto border border-blue-500 border-solid'>
                            ...
                            ? (<div>
                              <Link to={`${role === 'doctor' ? '/doctors/profile/me' : '/users/profile/me'}`}>
                                <figure className='w-[150px] h-[150px] rounded-full cursor-pointer'>
                                  <img src={user?.photo} className='w-full rounded-full bottom-[50px]ư' alt="UserImg" />
                                </figure>
                                  <h2>{user?.name}</h2>
                                  <h2>{user?.email}</h2>
                              </Link>
